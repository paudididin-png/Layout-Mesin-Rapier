...
function renderGrid(){ 
  ...
  for(const blockName of blockOrder){
    ...
    const machinesInBlock = []
    for(const range of ranges){
      for(let i = range.start; i <= range.end; i++){
        machinesInBlock.push(i)
      }
    }
    
    // Sort by machine number
    machinesInBlock.sort((a,b) => a - b)
    
    // Render machines in this block
    for(const i of machinesInBlock){
      const m = machines[i-1] || {id:i, constructId:null}
      const box = document.createElement('div')
      box.className = 'machine-box'
      box.title = `Mesin ${i} - Blok ${blockName}`
      box.textContent = i
      ...
      const isMachine = String(i)
      const matches = !filter || isMachine.includes(filter)
